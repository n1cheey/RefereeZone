...
const News: React.FC<NewsProps> = ({ user, onBack }) => {
  ...
  return (
    <Layout title="News" onBack={onBack}>
      {errorMessage && (
      ...
      {canManageNews && (
        <form onSubmit={handleCreatePost} className="mb-6 rounded-2xl border border-slate-100 bg-white p-5 shadow-sm space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-bold text-slate-900">Add News Post</h3>
          </div>
          <div>
            <label className="mb-1 block text-xs font-bold uppercase text-slate-500">YouTube Link</label>
            <input
              required
              value={youtubeUrl}
          ...
          <div>
            <label className="mb-1 block text-xs font-bold uppercase text-slate-500">Commentary</label>
            <textarea
              rows={4}
              value={commentary}
              ...
              placeholder="Write a short comment for this video..."
            />
          </div>
          {previewEmbedUrl && (
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <div className="text-xs text-slate-400">Posted by {item.createdByName}</div>
                    </div>
                    {canManageNews && (
                      <button
                        type="button"
